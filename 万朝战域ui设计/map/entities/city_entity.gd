...
const CENTRAL_CITY_ID := "central_capital"
const CENTRAL_CITY_MODEL_PATH := "res://map/assets/models/central_capital_hy3d31.glb"
const FOOTPRINT_FILL_RATIO := 0.9
const MIN_LABEL_HEIGHT := 3.05

## 城市角色枚举名称表，顺序必须与 MapCityData.CityRole 一致。
## @see res://map/data/city_data.gd → enum Role { FACTION_CAPITAL, CENTRAL_CAPITAL }
static var _CITY_ROLE_NAMES: PackedStringArray = [
	"FACTION_CAPITAL",
	"CENTRAL_CAPITAL",
]
...
func configure(p_city_data: MapCityData, map_data: DemoMapData) -> void:
	...
	position = map_data.get_surface_world_position(city_data.grid_position, 0.08)
	if is_node_ready():
		_update_label()
...
func _build_model_visual() -> bool:
	...
	var city_id_for_log := city_data.city_id if city_data != null else "UNKNOWN"
	var city_role_for_log: String = "UNKNOWN"
	if city_data != null:
		var ri: int = city_data.city_role
		if ri >= 0 and ri < _CITY_ROLE_NAMES.size():
			city_role_for_log = _CITY_ROLE_NAMES[ri]
		else:
			city_role_for_log = str(ri)
	
	if not ResourceLoader.exists(model_path, "PackedScene"):
		push_error(
			"[City] 模型资源不存在 | city_id=%s role=%s path=%s" % [
				city_id_for_log, city_role_for_log, model_path
			]
		)
		push_warning("城池模型不存在，使用程序化占位模型：%s" % model_path)
		return false
	var model_scene := load(model_path) as PackedScene
	if model_scene == null:
		push_error(
			"[City] 模型加载失败(PackedScene) | city_id=%s role=%s path=%s" % [
				city_id_for_log, city_role_for_log, model_path
			]
		)
		push_warning("城池模型加载失败，使用程序化占位模型：%s" % model_path)
		return false
	var model_instance := model_scene.instantiate()
	if not model_instance is Node3D:
		model_instance.queue_free()
		push_error(
			"[City] 根节点非Node3D | city_id=%s role=%s path=%s" % [
				city_id_for_log, city_role_for_log, model_path
			]
		)
	...
	var model_bounds := _calculate_model_bounds(model_root) as AABB
	if model_bounds.size.x <= 0.0 or model_bounds.size.z <= 0.0:
		model_root.queue_free()
		push_error(
			"[City] 模型边界无效 | city_id=%s role=%s path=%s bounds=%s" % [
				city_id_for_log, city_role_for_log, model_path, model_bounds
			]
		)
	...
	# 从模型自然尺寸计算占地格数
	var natural_tiles_x := int(ceil(model_bounds.size.x / _cell_size))
	var natural_tiles_z := int(ceil(model_bounds.size.z / _cell_size))
	city_data.natural_footprint_size = Vector2i(maxi(1, natural_tiles_x), maxi(1, natural_tiles_z))
	...
	var uniform_scale := target_width / maxf(model_width, 0.001)
	...
	# 存储模型世界 AABB（缩放后）
	city_data.model_world_aabb = AABB(
		model_root.position,
		model_bounds.size * uniform_scale
	)

	_disable_model_shadows(model_root)
	add_child(model_root)

	print("[City] %s footprint=%s natural=%s model_bounds=%.1fx%.1f scale=%.2f" % [
		city_data.city_id,
		city_data.footprint_size,
		city_data.natural_footprint_size,
		model_bounds.size.x, model_bounds.size.z,
		uniform_scale
	])
	return true
...
func _get_model_path() -> String:
	if city_data != null and city_data.city_id == CENTRAL_CITY_ID:
		if ResourceLoader.exists(CENTRAL_CITY_MODEL_PATH, "PackedScene"):
			return CENTRAL_CITY_MODEL_PATH
